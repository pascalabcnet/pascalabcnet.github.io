...
Модуль Coords. Что он делает - DrawLine
Модуль Turtle - изменение реализации. Он - основной
LightPT - много изменений
IndexMinBy, IndexMaxBy, DistinctBy
Возможность трассировать программы с модулем PT4
NewSet - ускорение множеств в 10 раз, изменение реализации
...
Функция `RealRange` возвращает последовательность вещественных значений между двумя заданными значениеями с указанным шагом. Важной её особенностью является то, что она не подвержена риску округления вещественных чисел. Так,
...
1 1.5 2 2.5 3
```
и исключения последнего значения вследствие ошибок округления не произойдёт.

#### Метод расширения последовательностей Scan 

Метод `Scan` аналогичен `Aggregate`, но возвращает элемент новой последовательности после обработки каждого элемента исходной последовательности.
...
Функция SetOf и метод расширения ToSet введены для создания встроенных множеств и преобразования во встроенные множества
...
#### Функция NumberFormat, процелура SetDecimalSeparator и параметр-разделитель в методах ToReal, ToReals и StrToReal

Данные изменения направлены на то чтобы считывать вещественные числа с произвольным разделителем между целой и дробной частью

```pascal
begin
  var nfi := NumberFormat(DecimalSeparator := ',');
  // В файле ar.txt пропускаются знаки табуляции и символы перехода на новую строку, вещественные хранятся в формате 2,14  3,24   6,3  7,891
