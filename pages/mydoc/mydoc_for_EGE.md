...
Окаймим алгоритм внешним циклом - скажем, от 1 до 30

```pascal
begin
  for var s1:=1 to 30 do
  begin
  var n := 1;
  var s := s1;
  while s < 51 do
...
30 32 
```
Из вывода хорошо видно, что наибольшим числом `s`, для которого выведется 64, является `25`
...
**Решение 2**. Используем стандартную функцию ToBase модуля School
