...
***Важно!***. Данный текст не рассматривает вопросы, связанные с сутью задачи. Он лишь описывает, как на PascalABC.NET сделать запись этих алгоритмов лучше, сохранив при этом эффективность. 
...
* В операторе `var a := ArrFill(m,0)` описывается и инициализируется массив a, при этом тип a автоматически выводится компилятором: это `array of integer`.
...
* Условная операция `if p = 0 then r[0] else r[m - p]` напоминает условный оператор, но является выражением, значение которого присваивается другой переменной. Основная идея её использования - однострочность конструкции инициализации переменной l. Сравним: 
...
Рассмотрим также два решения задачи 25 (формулировку см. здесь: [Демо ЕГЭ по информатике 2020](https://4ege.ru/index.php?do=download&id=10464)). Кратко также приводим формулировку ниже: 
...
***Решение 1 задачи 25 (Демо ЕГЭ 2020 года)***. Используется PascalABC.NET
```pascal
