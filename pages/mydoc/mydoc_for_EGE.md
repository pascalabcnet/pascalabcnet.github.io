...
* В операторе `var r := ArrFill(m,0)` описывается и инициализируется массив a, при этом тип a автоматически выводится компилятором: это `array of integer`. В любом случае вопрос о неинициализированном массива снимается (!)
